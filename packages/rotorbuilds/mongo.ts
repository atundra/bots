...
export const connect = (
  ...
  TE.tryCatch(
    ...
    (err: unknown) => err as MongoError,
  );

export const db =
  (name: string) =>
  (client: ConnectedMongoClient): Db =>
    client.db(name);
...
export const useMongo =
  (uri: string) =>
  <A>(use: (a: ConnectedMongoClient) => TE.TaskEither<Error, A>): TE.TaskEither<Error, A> =>
    TE.bracket(pipe(uri, client, connect), use, (client) => close(client));

export const find =
  <A>(q: FilterQuery<A>, o?: FindOneOptions<A extends A ? A : A>) =>
  (c: Collection<A>): TE.TaskEither<MongoError, A[]> =>
    TE.tryCatch(
      () => c.find(q, o!).toArray(),
      (e: unknown) => e as MongoError,
    );

export const insertMany =
  <A>(ds: RNEA.ReadonlyNonEmptyArray<OptionalId<A>>, o?: CollectionInsertManyOptions) =>
  (c: Collection<A>) =>
    TE.tryCatch(
...
export const deleteMany =
  <A>(q: FilterQuery<A>, o?: CommonOptions) =>
  (c: Collection<A>) =>
    TE.tryCatch(
      () => c.deleteMany(q, o),
      (e: unknown) => e as MongoError,
