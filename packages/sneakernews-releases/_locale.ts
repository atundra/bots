type Locale = 'en' | 'ru' | string;

export const locale = {
  welcome(locale: Locale = 'en', params: { SET_TIME_COMMAND: string }) {
    switch (locale) {
      case 'en':
  ...
  timeUpdated(locale: Locale = 'en', params: { RECEIVING_TIME: string }) {
    switch (locale) {
      case 'en':
  ...
  alreadyRegistered(
    locale: Locale = 'en',
    params: { SET_TIME_COMMAND: string }
  ) {
    switch (locale) {
      case 'en':
  ...
  headerMessage(locale: Locale = 'en', params: { RELEASES: string }) {
    switch (locale) {
      case 'en':
  ...
  notRegistered(locale: Locale = 'en') {
    switch (locale) {
      case 'en':
  ...
  noReleasesToday(locale: Locale = 'en') {
    switch (locale) {
      case 'en':
  ...
  setTimeZone(
    locale: Locale = 'en',
    params: {
      SELECT_TIMEZONE_COMMAND: string;
      GET_TIMEZONE_FROM_TIME_COMMAND: string;
    }
  ) {
    switch (locale) {
      // case 'en':
  ...
  selectTimeZone(locale: Locale = 'en', params: { SET_UTC_COMMAND: string }) {
    switch (locale) {
      default:
  ...
  timezoneUpdated(
    locale: Locale = 'en',
    params: { TIMEZONE: string; SET_TIME_COMMAND: string }
  ) {
    switch (locale) {
      default:
  ...
  noTimezone(locale: Locale = 'en', params: { SET_TIMEZONE_COMMAND: string }) {
    switch (locale) {
      default:
  ...
  sendMeYourTime(
    locale: Locale = 'en',
    params: { SET_TIMEZONE_FROM_TIME_COMMAND: string }
  ) {
    switch (locale) {
      default:
